...
class GCPUserController extends Controller
{
     ...
     public function getVmList()
     {
          try {
               ...
               return response()->json(['message' => $e->getMessage->response()]);
          }
     }
     ...
     public function vmStartAndStop(Request $request)
     {
          ...
          try {
               ...
               if ($token) {
                    ...
                    $vmUsed = VMUsed::where('vm_id', $updateStopStart->id)->orderBy('id', 'DESC')->first();
                    if ($request->action == 'stop' && $vmUsed) {
                         ...
                         $vmUsed->used_min = $total_minutes;
                         $vmUsed->save();
                         //  VMUsed::where('vm_id', $updateStopStart->id)->orderBy('id', 'DESC')->update(['used_min' => $total_minutes])->first();
                    }
               }
               DB::commit();
